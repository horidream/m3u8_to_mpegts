  ...
  for (i = 0; i < lines.length; i++) {
    ...
    if (currentLine.match(/^#EXT-X-STREAM-INF/i)) {
      ...
      mediaPlaylist = {
        ...
        bandwidth: parseInt(
          currentLine.match(/BANDWIDTH=\d+/i)[0].split("=")[1]
        ),
        segments: [],
        IV: undefined,
        keyURI: undefined,
...
function parseMediaPlaylist(playlist, done, rootUri, cwd) {
  ...
  fetch.fetchUrl(playlist.uri, function (err, meta, body) {
    // if body is undefined, read the uri locally
    var lines = (body || fs.readFileSync(playlist.uri, "utf-8"))
        .toString()
        .split("\n"),
      i,
      currentLine;
    ...
    fs.writeFileSync(
      path.resolve(cwd, "playlist.m3u8"),
      manifestLines.join("\n")
    );
...
function update(rootUri) {
  ...
  fetch.fetchUrl(playlist.uri, function (err, meta, body) {
    var newPlaylistLines = body.toString().split("\n"),
      ...
      resource;
    if (
      lastSegmentUri === undefined &&
      this.segments[this.segments.length - 1].endList
    ) {
