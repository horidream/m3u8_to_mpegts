...
function getIt(options, done) {
  ...
  fetch.fetchUrl(uri, function getPlaylist(err, meta, body) {
    ...
    if (err) {
      // if fetch can't fetch the data, try read it locally.
      let data = fs.readFileSync(uri, "utf-8");
      if (!data) {
    ...
    if (body.toString().match(/#EXTINF/)) {
      ...
      oldLength = 1;
      parse.parseMediaPlaylist(
        mediaPlaylist,
        doneParsing,
        path.dirname(options.uri),
        cwd
      );
    } else {
      ...
      fs.writeFileSync(
        path.resolve(cwd, playlistFilename),
        masterPlaylist.manLines.join("\n")
      );
      // parse the mediaplaylists for segments and targetDuration
      for (i = 0; i < mediaPlaylists.length; i++) {
        parse.parseMediaPlaylist(
          masterPlaylist.medPlaylists[i],
          doneParsing,
          path.dirname(masterPlaylist.uri),
          cwd
        );
      }
      ...
      if (mediaPlaylist) {
        ...
        if (masterPlaylist.mediaPlaylists.length === oldLength) {
          ...
          for (var i = 0; i < masterPlaylist.mediaPlaylists.length; i++) {
            //in masterPlaylist.mediaPlaylists we got all the available playlists
            // console.log('Bandwidth: ' + masterPlaylist.mediaPlaylists[i].bandwidth);
            var curPlaylistBandwith =
              masterPlaylist.mediaPlaylists[i].bandwidth;
            if (
              curPlaylistBandwith <
              masterPlaylist.mediaPlaylists[lowQualityIt].bandwidth
            ) {
              //Simply finding the minimum bandwith playlist
              lowQualityIt = i;
            }
            if (
              curPlaylistBandwith >
              masterPlaylist.mediaPlaylists[highQualityIt].bandwidth
            ) {
              //Simply finding the maximum bandwith playlist
              highQualityIt = i;
            }
          }
      ...
      playlist.download(
        ...
        function () {
          ...
          if (mediaPlaylist) {
            ...
            playlistsFinished == masterPlaylist.mediaPlaylists.length
          ) {
            console.log("shutting down");
            done();
          }
    ...
    function setupDownload(type) {
      ...
      for (i = 0; i < pl.length; i++) {
        ...
        downloadFunction(
          ...
          pl[i].bandwidth,
          function () {
            console.log("finish setupDownload"); /*process.exit();*/
          },
          finishedDownloadingSegment
        );
        if (!pl[i].endList) {
